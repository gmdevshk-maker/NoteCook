...
from backend import library, ollama, rag, report

# 라이브러리에 추가 가능한 문서 확장자 (파일 대화상자/드래그앤드롭 공통).
ALLOWED_EXT = (".pdf", ".docx", ".xlsx", ".hwpx", ".txt", ".md")
...
class Api:
    ...
    def add_documents(self, notebook_id: str) -> dict:
        ...
        try:
            paths = self._window.create_file_dialog(
                webview.OPEN_DIALOG,
                allow_multiple=True,
                file_types=("문서 파일 (*.pdf;*.docx;*.xlsx;*.hwpx;*.txt;*.md)", "모든 파일 (*.*)"),
            )
    ...
    def open_path(self, path: str) -> dict:
        """내보내기로 생성한 파일을 Windows 기본 연결 앱으로 연다."""
        try:
    ...
    def export_report(self, question: str, doc_ids: list, model: str, fmt: str) -> dict:
        """
        선택 문서 범위로 리포트를 생성해 docx/pptx/xlsx/txt 파일로 저장한다.
        먼저 저장 위치를 묻는 대화상자를 띄우고, 사용자가 고른 경로에 파일을 만든다.
        (생성은 RAG 검색 + 구조화 LLM 호출이라 수초~수십초 걸릴 수 있어 호출 측에서
        대기 표시를 한다. 분석과 달리 스트리밍은 없다.)
        """
        question = (question or "").strip()
        fmt = (fmt or "").lower()
        if fmt not in report.FORMATS:
            return {"error": f"지원하지 않는 형식입니다: {fmt}"}
        if not question:
            return {"error": "리포트로 만들 내용을 프롬프트에 입력하세요."}
        if not doc_ids:
            return {"error": "분석할 문서를 먼저 선택하거나 추가하세요."}
        if not model or rag.EMBED_MODEL == model:
            return {"error": "생성 모델을 선택하세요."}
        if not self._window:
            return {"error": "창 참조가 없습니다."}

        # 저장 대화상자 (기본 파일명: 질문 앞부분 → 안전한 파일명).
        ext = report.FORMATS[fmt]
        default_name = self._safe_filename(question) + ext
        try:
            result = self._window.create_file_dialog(
                webview.SAVE_DIALOG,
                save_filename=default_name,
                file_types=(f"{fmt.upper()} 파일 (*{ext})", "모든 파일 (*.*)"),
            )
        except Exception as e:
        ...
        ollama.load_model(model)
        try:
            res = report.export(question, list(doc_ids), model, fmt, path,
                                embed_model=rag.EMBED_MODEL)
            return res
        except requests.RequestException as e:
            return {"error": f"모델 호출 실패: {e}. Ollama 실행 및 모델 상태를 확인하세요."}
        except Exception as e:
            return {"error": str(e)}

    @staticmethod
    def _safe_filename(text: str, limit: int = 40) -> str:
        """질문 문자열을 Windows 파일명으로 안전하게 변환."""
        name = re.sub(r'[\\/:*?"<>|\n\r\t]', " ", text).strip()
        name = re.sub(r"\s+", " ", name)[:limit].strip()
        return name or "NoteCook_리포트"
