...
- PDF / DOCX / XLSX / HWPX(한글) / TXT / Markdown 문서를 노트북 단위로 분류·관리합니다.
- 로컬 LLM(**Gemma 4 via Ollama**)으로 문서에 대해 질의응답하며, 답변에 **출처(문서·페이지)를 인용**합니다.
- 분석 결과를 **Word(docx) / PowerPoint(pptx) / Excel(xlsx) / 텍스트(txt) 파일로 내보낼 수 있습니다.** (요약·섹션·표·차트 자동 구성, pptx·xlsx는 네이티브 차트)
...
| 문서 파싱 | `pypdf` (PDF), `python-docx` (DOCX), `openpyxl` (XLSX, 시트별 추출), 표준 ZIP/XML (HWPX), 내장 텍스트 (TXT/MD) |
| 문서 내보내기 | `python-docx` (DOCX), `python-pptx` (PPTX·네이티브 차트), `openpyxl` (XLSX·네이티브 차트), 내장 (TXT) — LLM 은 구조화 JSON 만 생성, 파일 조립은 코드가 담당 |
...
좌측 노트북 트리에서 PDF/DOCX/XLSX/HWPX/TXT 문서를 폴더 또는 개별 단위로 선택해
...
![파일별 내보내기](cap/cap_06.png)

프롬프트 입력 후 **내보내기** 버튼에서 출력 형식(**Word / PowerPoint / Excel / 텍스트**)을 선택하면,
선택된 문서를 RAG로 검색·분석한 결과를 해당 문서 파일로 저장합니다.

- LLM(Gemma)은 **요약·섹션·표·차트**를 구조화된 JSON 으로만 생성하고, 실제 파일 조립은 코드가 결정론적으로 처리해 형식 안정성을 확보합니다.
- **PowerPoint·Excel** 은 편집 가능한 **네이티브 차트**로, Word·텍스트는 데이터 표로 출력됩니다.
- 문맥에 실제로 존재하는 수치만 사용하며, 데이터가 없으면 표·차트는 자동 생략되어 **수치 환각을 방지**합니다.
- 모든 출력은 **외부 API·인터넷 없이 오프라인**으로 생성됩니다.
